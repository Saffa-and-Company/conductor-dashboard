...
export async function GET(request: NextRequest) {
  ...
  switch (type) {
    ...
    case 'pipelines': {
      const pipelineConditions = [...conditions]
      const pipelineParams = [...params]
      pipelineConditions.unshift('wp.workspace_id = ?')
      pipelineParams.unshift(workspaceId)
      const pipelineWhere = pipelineConditions.length > 0
        ? `WHERE ${pipelineConditions.join(' AND ').replace(/\bcreated_at\b/g, 'pr.created_at')}`
        : ''
